...
MuseAI is a local Obsidian plugin fork based on Claudian. It embeds provider-backed chat runtimes in a sidebar and inline-edit flow. Claude is the default provider, while Codex, OpenCode, and Pi join the same conversation model through `Conversation.providerId` plus provider-owned `providerState`.

## Architecture Status

- Product status: MuseAI keeps Claudian's multi-provider architecture. Claude is the full-feature provider. Codex is opt-in and currently supports send, stream, cancel, resume, history reload, fork, plan mode, image attachments, inline edit, `#` instruction mode, `$` skills, and subagents. Unsupported or gated Codex surfaces are rewind, runtime-discovered provider commands, in-app MCP management, and Claude plugin integration.
...
| `.claudian/claudian-settings.json` | Shared MuseAI app settings plus provider-specific configuration |
| `.claude/mcp.json` | MuseAI-managed MCP servers for Claude |
