...
describe('ProviderSettingsCoordinator', () => {
  ...
  describe('projectActiveProviderState', () => {
    ...
    it('uses the saved Codex model when the active global projection belongs to OpenCode', () => {
      const settings: Record<string, unknown> = {
        ...
        savedProviderEffort: { codex: 'medium', opencode: 'low' },
        savedProviderServiceTier: { codex: 'fast', opencode: 'default' },
        savedProviderThinkingBudget: { codex: 'off', opencode: 'low' },
      };

      const snapshot = ProviderSettingsCoordinator.getProviderSettingsSnapshot(settings, 'codex');

      expect(snapshot.model).toBe(DEFAULT_CODEX_PRIMARY_MODEL);
      expect(snapshot.effortLevel).toBe('medium');
      expect(snapshot.serviceTier).toBe('fast');
    });
