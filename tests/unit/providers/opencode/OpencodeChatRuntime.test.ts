...
describe('OpencodeChatRuntime', () => {
  ...
  it('rejects ACP file reads and writes outside the workspace root', () => {
    const runtime = new OpencodeChatRuntime(createMockPlugin());
    const cwd = path.resolve('/tmp/claudian-test-vault');
    const insidePath = path.resolve(cwd, 'notes/today.md');

    (runtime as any).sessionCwds.set('session-1', cwd);

    expect(() => (runtime as any).resolveSessionPath('session-1', path.resolve('/tmp/outside.md'))).toThrow(
      'OpenCode file access is limited to the current workspace.',
    );
    expect(() => (runtime as any).resolveSessionPath('session-1', '../outside.md')).toThrow(
      'OpenCode file access is limited to the current workspace.',
    );
    expect((runtime as any).resolveSessionPath('session-1', insidePath)).toBe(insidePath);
    expect((runtime as any).resolveSessionPath('session-1', 'notes/today.md')).toBe(insidePath);
  });
