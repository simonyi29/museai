...
describe('CodexChatRuntime', () => {
    ...
    it('sends initialize and initialized', async () => {
      ...
      expect(mockTransportRequest).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          clientInfo: { name: 'museai', version: '1.0.0' },
        }),
      );
  ...
  describe('query - new thread', () => {
    ...
    it('ignores a non-Codex model override instead of sending it to Codex', async () => {
      await collectChunks(runtime.query(
        createTurn('hi'),
        undefined,
        { model: 'opencode:deepseek/deepseek-v4-pro' } as any,
      ));
